...
const BACKGROUND_CHECKBOX_MUTANT = '.setChecked(false); \n'
...
const BACKGROUND_EDITTEXT_REGEX = /(EditText)?\s*([A-Za-z\d_]+)\s*=\s*\(?\s*\(EditText\)\s*findViewById\(([A-Za-z\d._]+)\)\s*\)?;/;
const BACKGROUND_SPINNER_REGEX = /(Spinner)?\s*([A-Za-z\d_]+)\s*=\s*\(?\s*\(Spinner\)\s*findViewById\(([A-Za-z\d._]+)\)\s*\)?;/;
const BACKGROUND_CHECKBOX_REGEX = /(CheckBox)?\s*([A-Za-z\d_]+)\s*=\s*\(?\s*\(CheckBox\)\s*findViewById\(([A-Za-z\d._]+)\)\s*\)?;/;
...
module.exports.BACKGROUND_CHECKBOX_MUTANT = BACKGROUND_CHECKBOX_MUTANT;
...
module.exports.BACKGROUND_CHECKBOX_REGEX = BACKGROUND_CHECKBOX_REGEX;
